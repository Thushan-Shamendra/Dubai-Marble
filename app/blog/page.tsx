      ...
      <section className="py-24 bg-[#F7F9FC]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <AnimatedSection className="text-center max-w-2xl mx-auto mb-14">
            ...
            <h2 className="text-3xl font-black text-[#10233F]">Professional Stone Care Insights</h2>
          </AnimatedSection>
              ...
              <AnimatedSection key={a.title} delay={i * 0.1}>
                <div className="group bg-white rounded-2xl overflow-hidden border border-[#D9E2EC] shadow-sm hover:shadow-xl hover:border-[#1557B0]/40 transition-all hover:-translate-y-1.5 flex flex-col h-full">
                  ...
                  <div className="p-6 flex flex-col flex-grow">
                    <div className="flex items-center gap-4 text-[11px] text-[#98A2B3] mb-3">
                      <span className="flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5 text-[#1557B0]" /> {a.date}</span>
                      <span className="flex items-center gap-1.5"><User className="w-3.5 h-3.5 text-[#1557B0]" /> {a.author}</span>
                    </div>
                    <h2 className="font-bold text-[#10233F] text-base mb-3 leading-snug group-hover:text-[#1557B0] transition-colors">
                      {a.title}
                    </h2>
                    <p className="text-xs text-[#667085] leading-relaxed mb-5 flex-grow">{a.excerpt}</p>
                    <button
                      onClick={() => alert("Full article coming soon in our newsletter!")}
                      className="text-xs font-bold text-[#1557B0] hover:text-[#2F80ED] flex items-center gap-1.5 hover:gap-3 transition-all mt-auto"
                    >
